...
referenz <- data.frame(titl,template,parameter,connection,label,l_x,l_y,h_c,h_t,b_t,mat_t,mat_c,n_v_x,t_v)
...
new_var <- list(6.75,9.75,0.675,2.025,0.07,0.12,0.24,0.4,0.2,0.3,"GL20h","GL32h","C25/30","C45/55",2,5,0.02,0.05) # Nur mit der Liste werden verschiedeneh Datentypen unterstÃ¼tzt
...
new_parameter <- c("Balkenlänge","Balkenlänge","Balkenabstand","Balkenabstand",
                   ...
                   "Betongüte","Betongüte","Anzahl Kerven","Anzahl Kerven",
                   "Kerventiefe","Kerventiefe")
...
#input_screw1[,"con_parameter"] <- "VG-48-7,5x165 1"
#input_screw1[,"connection"] <- "screws"
#input_screw1[,"n_v_x"] <- 24
#input_screw1[,"t_v"] <- 0.0075
#
## Verbindungsmittel werden in Listen gespeichert, pro Fall eine Liste
#n_v_screw1 <- lapply(input_screw1[,"n_v_x"], function(x) rep(2,x))
#k_s_pm_screw1 <- lapply(input_screw1[,"n_v_x"], function(x) rep(240,x)) # k_s_pm = Verschiebungsmodul pro Meter Kervenbreite oder pro einzelner Schraube (als Vektor mit einem Wert pro VM-Reihe) in kN/mm
#x_v_eff_screw1 <- lapply(input_screw1[,"n_v_x"], function(x) rep(0.165,x)) # x_v_eff = Bei Kerven die Kervenbreite [m] oder effektive Einbindetiefe der Schraube ins Holz (immer als Vektor mit einem Wert pro VM-Reihe) [m]
...
#
#
#input_screw1[,"f_v_ax_k"]  <-  16  # f_v_ax_k = Ausziehwiderstand der Schrauben in N/mm?
#input_screw1[,"f_v_tens_k"]  <- 17 # f_v_tens_k = Zugwiderstand der Schrauben in kN
#input_screw1[,"alpha_v"] <- 45   # alpha_v = Einschraubwinkel in deg
...
#input_screw2[,"con_parameter"] <- "VG-48-7,5x165 2"
#input_screw2[,"connection"] <- "screws"
#input_screw2[,"n_v_x"] <- 18
#input_screw2[,"t_v"] <- 0.0075
#
## Verbindungsmittel werden in Listen gespeichert, pro Fall eine Liste
#n_v_screw2 <- lapply(input_screw2[,"n_v_x"], function(x) rep(2,x))
#k_s_pm_screw2 <- lapply(input_screw2[,"n_v_x"], function(x) rep(240,x)) # k_s_pm = Verschiebungsmodul pro Meter Kervenbreite oder pro einzelner Schraube (als Vektor mit einem Wert pro VM-Reihe) in kN/mm
#x_v_eff_screw2 <- lapply(input_screw2[,"n_v_x"], function(x) rep(0.165,x)) # x_v_eff = Bei Kerven die Kervenbreite [m] oder effektive Einbindetiefe der Schraube ins Holz (immer als Vektor mit einem Wert pro VM-Reihe) [m]
...
#a_v_screw2 <- lapply(input_screw2[,"n_v_x"], function(x) cumsum(rep(0.15,x))) # a_v = Abstaende der Verbindungsmittel immer bezogen auf das Traegerende [m]. cumsum() Schreibweise empfohlen
#
#
#input_screw2[,"f_v_ax_k"]  <-  16  # f_v_ax_k = Ausziehwiderstand der Schrauben in N/mm?
...
###### Zusammenfuehren
#
#input <- rbind(input_screw1,input_screw2)
#
#n_v <- c(n_v_screw1,n_v_screw2)
#
#k_s_pm <- c(k_s_pm_screw1,k_s_pm_screw2)
#x_v_eff <- c(x_v_eff_screw1,x_v_eff_screw2)
#l_v <- c(l_v_screw1,l_v_screw2)
#a_v <- c(a_v_screw1,a_v_screw2)
