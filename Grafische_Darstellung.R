...
# Nachweisnamen für Plot
names_eta <- c("ETA,sig,t,u,0","ETA,sig,t,u,3-7","ETA,sig,t,u,inf","ETA,sig,c,o,0","ETA,sig,c,o,3-7","ETA,sig,c,o,inf","ETA,tau,t,0","ETA,tau,t,3-7","ETA,tau,t,inf","ETA,V,v,0","ETA,V,v,3-7","ETA,V,v,inf","ETA,w,0","ETA,w,inf") # enthält NW-Bezeichnungen
# Liste der VBM, die unterschieden werden
names_con <- c("Klebung","Kerve","ASSY plus VG","VB-48-7,5 x 100 (45°/135°)")
# Liste der Nachweise die geplottet werden können
...
plot_eta_lp <- function(sim_data,names_par,sim_param,nachweis,names_con){
  ...
  eta <- data.frame()     # Dataframe für Ergebnisdaten
  ref <- data[which(data$parameter %like% "Referenz"),]
  i   <- 1                # Laufvariable
  x_k <- 0                # x-Koordinate im Plot (vertikale Position)
  rep <- 2                # Zähler für untersuchte Zeitpunkte 0..2
  eta_time <- c("t=0","t=3-7","t=inf")  # Label für Zeitpunkte
  #eta_col  <- c("darkmagenta","mediumorchid2","hotpink1")  # Färbung des Plots für Zeitpunkte
  #eta_col  <- c("aquamarine","aquamarine2","aquamarine4")
  eta_col  <- c("darkred","darkorange3","darkorange1")
  
  # Überschrift für Plot
  ...
  # Spaltenbezeichnungen der Nachweise in Ergebnisdatei - genaue Übereinstimmung notwendig
  ...
  # Erstelle Listen mit Spaltennummern die für die Konfiguration betrachten werden
  n_eta <- c()
  ...
  if(nachweis=="sig_t"){
    ...
    rep <- 1
  } else return("Nachweisbezeichnung prüfen")
    
  # Filtern der Maximal und Minimawerte für Lollipop-Plot
  for(name in names_par){
    for(j in 0:rep){
      ...
      # Ermittlung des zugehörgen Werts des Parameters
      ...
      # Hilfskoordinate für Plot: Vertikaler Abstand zwischen Linien
      eta[i+j,"x"]         <- x_k-j*0.25
      eta[i+j,"eta_col"]   <- eta_col[j+1]
      # Refernzwerte für Plot
      eta[i+j,"eta_ref"] <- ref[num_eta[j+1]]
      # Test: Welche Daten werden ausgelesen
      print(eta[i+j,"param_min"])
      print(eta[i+j,"eta_min"])
      }
    i <-i+rep+1   # Laufvariable für Nachweiszeitpunkte (bei GZG ist rep=2, bei GZT-Nachweisen rep=3)
    x_k <- x_k-1  # Laufvariable für Hilfskoordinate x
  }

  # Geometrische Hilfvariablen für Positionierung der grafischen Elemente
...
## Funktion Risshöheniteration Beton
...
plot_eta_lp2 <- function(sim_data,names_par,sim_param,nachweis,names_con){
  ...
  for(name in names_par){
    for(j in 0:rep){
      ...
      # Bemessungszwitpunkt
      eta[i+j,"eta_time"]  <- eta_time[j+1]
      # Ermittlung der mininaölen und maximalen Ausnutzungsgrade
      eta[i+j,"eta_min"]   <- min(data[which(data$parameter %like% name),num_eta[j+1]],na.rm = FALSE)
      eta[i+j,"eta_max"]   <- max(data[which(data$parameter %like% name),num_eta[j+1]],na.rm = FALSE)
      # Ermittlung des zugehörgen Werts des Parameters
      eta[i+j,"param_min"] <- data$label[which(data$parameter %like% name & data[num_eta[j+1]] == min(data[which(data$parameter %like% name),num_eta[j+1]],na.rm = FALSE))]
